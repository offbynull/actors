package com.offbynull.peernetic.networktools.netty.handlers.common;
...
public abstract class AbstractTransformArrivingHandler extends MessageToMessageDecoder<Object> {
    ...
    protected final void decode(ChannelHandlerContext ctx, Object msg, List<Object> out) throws Exception {
        ...
        if (msg instanceof AddressedEnvelope) {
            ...
            Object encoded = transform(ctx, recipient, sender, envelopeMsg.content());
            
            res = new DefaultAddressedEnvelope<>(encoded, envelopeMsg.recipient(), envelopeMsg.sender());
        } else {
            res = transform(ctx, ctx.channel().localAddress(), ctx.channel().remoteAddress(), msg);
        }
        
        out.add(res);
    }
    
    protected abstract Object transform(ChannelHandlerContext ctx, SocketAddress localAddress, SocketAddress remoteAddress, Object obj);
}
