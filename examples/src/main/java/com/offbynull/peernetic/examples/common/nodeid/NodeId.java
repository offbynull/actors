...
public final class NodeId implements Serializable {
     ...
     */
    private NodeId(BigInteger data, BigInteger limit) {
     ...
     */
    public NodeId increment() {
        return add(this, new NodeId(new byte[] {1}, limit.toByteArray()));
    }
     ...
     */
    public NodeId decrement() {
        return subtract(this, new NodeId(new byte[] {1}, limit.toByteArray()));
    }
     ...
     */
    public static NodeId add(NodeId lhs, NodeId rhs) {
        ...
        NodeId addedId = new NodeId(added, lhs.limit);
     ...
     * @throws IllegalArgumentException if the limit from {@code lhs} doesn't match the limit from {@code rhs}
     */
    public static NodeId subtract(NodeId lhs, NodeId rhs) {
        Validate.notNull(lhs);
        Validate.notNull(rhs);
        Validate.isTrue(lhs.limit.equals(rhs.limit));
        ...
        NodeId subtractedId = new NodeId(subtracted, lhs.limit);
     ...
     */
    public static int comparePosition(NodeId base, NodeId lhs, NodeId rhs) {
        Validate.notNull(base);
        Validate.notNull(lhs);
        Validate.notNull(rhs);
        Validate.isTrue(base.limit.equals(lhs.limit) && base.limit.equals(rhs.limit));
        
        NodeId lhsOffsetId = subtract(lhs, base);
        NodeId rhsOffsetId = subtract(rhs, base);
     ...
     */
    public boolean isWithin(NodeId lower, boolean lowerInclusive, NodeId upper, boolean upperInclusive) {
